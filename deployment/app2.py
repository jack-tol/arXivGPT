...
async def send_query_actions():
    if session_active:  # Only send actions if the session is active
        actions = [
            cl.Action(name="ask_new_question", value="new_question", description="Ask a new question on the same paper"),
            cl.Action(name="ask_about_new_paper", value="new_paper", description="Ask about a new paper"),
            cl.Action(name="exit_session", value="exit", description="Exit")
        ]
...
async def on_ask_about_new_paper(action):
    global selected_doc_id, session_active
...
async def on_exit_session(action):
    ...
    session_active = False  # Update the session state to inactive
    await cl.Message(content="Session ended. Goodbye!").send()
    await action.remove()  # Remove the exit button after use

async def ask_for_query(selected_doc_id):
